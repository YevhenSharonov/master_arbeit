...
# hvmv substations, grant (oeuser) and comment on table
    create_table_hvmv_substations = PostgresOperator(
        task_id='hvmv_substations',
...
# WAY: create view of way substations, grant (oeuser)
    create_view_way_substation = PostgresOperator(
...
# WAY: create view of way substations with 110kV, grant (oeuser)
    create_view_way_substation_110kv = PostgresOperator(
        task_id='create_view_way_substation_100kV',
        postgres_conn_id='postgres_oedb',
        sql='''
...
# WAY: create view of substations without 110kV, grant (oeuser)
    create_view_way_substation_without_110kv = PostgresOperator(
        task_id='create_view_way_substation_100kV',
        postgres_conn_id='postgres_oedb',
        sql='''
...
# NODE: create view of 110kV node substations, grant (oeuser)
    create_view_node_substation_110kv = PostgresOperator(
...
# LINES 110kV: create view of 110kV lines, grant (oeuser)
    create_view_lines_substation_110kv = PostgresOperator(
...
# grant (oeuser)
    create_view_way_substations_without_110kv_intersected_by_110kv_line = PostgresOperator(
...
# drop view substation_100kV and create new view substation_100kV, grant (oeuser)
    drop_create_view_substation_110kv = PostgresOperator(
...
# create view summary_total that contains substations without any filter, grant (oeuser)
    create_view_summary_total = PostgresOperator(
...
# create view that filters irrelevant tags, index gist (geom), grant (oeuser)
    create_view_filters_irrelevant_tags = PostgresOperator(
...
# eliminate substation that are not within VG250, grant (oeuser)
    create_view_substation_not_within_vg250 = PostgresOperator(
...
# create view with buffer of 75m around polygons, grant (oeuser)
    create_view_buffer_75m_around_polygons = PostgresOperator(
...
# create second view with same data to compare, grant (oeuser)
    create_second_view_with_same_data = PostgresOperator(
        task_id='create_second_view_with_same_data_to_compare',
        postgres_conn_id='postgres_oedb',
        sql='''
...
# create view to eliminate smaller substations where buffers intersect, grant (oeuser)
    create_view_eiliminate_smaller_substations = PostgresOperator(
...
# filter those substations and create final_result, grant (oeuser)
    create_view_filter_substations_final_result = PostgresOperator(
