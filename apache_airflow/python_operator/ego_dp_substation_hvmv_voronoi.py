...
# scenario log, set id as subst_id, index GIST (geom), scenario log
def set_id_as_subst_id(**kwargs):
    ...
    with engine.connect() as connection:
        result = connection.execute('''
                 SELECT scenario_log('eGo_DP', 'v0.4.5','input','model_draft','ego_grid_hvmv_substation','ego_dp_substation_hvmv_voronoi.sql',' ');
	         ...
	         DROP COLUMN IF EXISTS geom CASCADE,
	          ...
	          WHERE t1.subst_id = t2.subst_id;
                 ...
                 SELECT scenario_log('eGo_DP', 'v0.4.5','input','model_draft','ego_boundaries_bkg_vg250_6_gem_clean','ego_dp_substation_hvmv_voronoi.sql',' '); ''')


# Gemeindeschlüssel, scenario log
    ...
    with engine.connect() as connection:
        result = connection.execute('''
	          ...
	          WHERE t1.subst_id = t2.subst_id;
                 ...
                 SELECT scenario_log('eGo_DP', 'v0.4.5','output','model_draft','ego_grid_hvmv_substation','ego_dp_substation_hvmv_voronoi.sql',' '); ''')


# create dummy points for voronoi calculation, insert points around germany (18 Points), index GIST (geom), grant (oeuser),
# metadata, select description, scenario log
def create_dummy_points_for_voronoi_calculation(**kwargs):
    ...
    with engine.connect() as connection:
        result = connection.execute('''
                 ...
                 SELECT scenario_log('eGo_DP', 'v0.4.5','output','model_draft','ego_grid_hvmv_substation_dummy','ego_dp_substation_hvmv_voronoi.sql',' '); ''')


# voronoi polygons with eucldean distance, set table, index GIST (geom), grant (oeuser)
# delete dummy points from substations and voronoi (18 Points), metadata, select description, scenario log
def voronoi_polygons_with_eucldean_distance(**kwargs):
...
dag_params = {
    'dag_id': 'ego_dp_substation_hvmv_voronoi',
...
with DAG(**dag_params) as dag:
    ...
    set_id_as_subst_id = PythonOperator(
        task_id='set_id_as_subst_id',
        python_callable=set_id_as_subst_id
    )

    gemeindeschluessel = PythonOperator(
        task_id='gemeindeschluessel',
        python_callable=gemeindeschluessel
    )

    create_dummy_points_for_voronoi_calculation = PythonOperator(
        task_id='create_dummy_points_for_voronoi_calculation',
        python_callable=create_dummy_points_for_voronoi_calculation
    )

    voronoi_polygons_with_eucldean_distance = PythonOperator(
        task_id='voronoi_polygons_with_eucldean_distance',
        python_callable=voronoi_polygons_with_eucldean_distance
    )

    trigger_next_dag =  TriggerDagRunOperator(
    trigger_dag_id='ego_dp_substation_ehv_voronoi', 
    task_id='add_dummy_points'
    )

    set_id_as_subst_id >> gemeindeschluessel >> create_dummy_points_for_voronoi_calculation >> voronoi_polygons_with_eucldean_distance >> trigger_next_dag
